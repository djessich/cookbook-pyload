...
action :install do
  ...
  # a valid PyloadNG (>= 0.5.0) version
  unless pyload_next?(new_resource.version)
    # Python 2 packages are not available for RHEL 9+
    raise 'Install method source is only supported for version >= 0.5.0 on RHEL 9+.' if platform_family?('rhel') && node['platform_version'].to_i >= 9

    # Python 2 packages are not available for OpenSUSE 15.4+
    raise 'Install method source is only supported for version >= 0.5.0 on OpenSUSE 15.4+.' if platform_family?('suse') && node['platform_version'] >= '15.4'
  end
